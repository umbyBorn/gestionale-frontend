...
import {
  ...
  getTesserati, caricaVerbale, eliminaVerbale,
} from '../services/api';
                      ...
                      <button key={giorno}
                        ...
                        {haEventi && (
                          <span className="flex gap-0.5 mt-0.5">
                            {eventiGiorno.slice(0, 3).map((a, idx) => (
                              <span key={idx} className={`w-1.5 h-1.5 rounded-full ${isSelezionato ? (statoColore[a.stato]?.split(' ')[0] || 'bg-blue-400') : 'bg-yellow-300'}`} />
                            ))}
                          </span>
                        )}
                      </button>
